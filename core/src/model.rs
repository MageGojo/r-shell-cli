...
impl PersistedWorkspace {
    pub fn normalized(mut self) -> Self {
        // Keep SSH and ADB connections; drop any other (legacy) protocol.
        self.connections.retain(|connection| {
            connection.protocol.eq_ignore_ascii_case("SSH")
                || connection.protocol.eq_ignore_ascii_case("ADB")
        });

        for connection in &mut self.connections {
            connection.protocol = if connection.protocol.eq_ignore_ascii_case("ADB") {
                "ADB".to_string()
            } else {
                "SSH".to_string()
            };
